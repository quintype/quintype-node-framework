...
describe("Isomorphic Handler", function () {
  ...
  describe("Prerender server", function () {
    ...
    newApp.get("/*", (req, res) => {
      ...
      return res.status("200").json({ text: "Prerender", queryParams: req.query });
    });
    ...
    it("Should run prerender servre if i pass any other server top of it", function (done) {
      supertest(newServer)
        ...
        .expect(200)
        .then((res) => {
          assert.equal(res.body.text, "Prerender");
        })
        .then(done, done)
        .finally(() => {
          newServer.close();
    ...
    it("Should run prerender servre if i pass any other server top of it", function (done) {
      supertest(newServer)
        .get("/http://localhost:3000/?prerender=true")
        .expect(200)
        .then((res) => {
          assert.deepEqual(res.body.queryParams, {});
        })
        .then(done, done)
        .finally(() => {
          newServer.close();
